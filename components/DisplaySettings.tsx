  ...
  return (
    <div className="relative" ref={menuRef}>
      <button 
        ...
        className="text-sm text-slate-500 hover:text-teal-600 dark:text-slate-400 dark:hover:text-teal-400 flex items-center gap-1 font-medium transition-colors p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
        title="顯示設定"
      >
        <Monitor size={18} />
                ...
                {fontSizes.map((size) => (
                    <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-left transition-colors ${
                            ...
                            ? 'bg-teal-50 dark:bg-teal-500/15 text-teal-700 dark:text-teal-300 border border-teal-100 dark:border-teal-500/30' 
                            : 'hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300'
                        }`}
                    >
                        <div className="flex items-center gap-3">
                            <Type size={16} className={currentFontSize === size.id ? 'text-teal-500' : 'text-slate-400'} />
                            <span className="text-sm font-medium">{size.label}</span>
                        </div>
                        {currentFontSize === size.id && <Check size={16} className="text-teal-600 dark:text-teal-400" />}
                    </button>
                ))}
            </div>
        </div>
