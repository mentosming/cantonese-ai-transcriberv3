...
const FileUpload: React.FC<FileUploadProps> = ({ onFileSelect, selectedFile, onClear, disabled }) => {
  ...
  if (selectedFile) {
    ...
    return (
      <div className="w-full bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm transition-colors">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="min-w-0">
              <h3 className="font-medium text-slate-900 dark:text-slate-100 truncate">{selectedFile.name}</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">{sizeInMB} MB • {selectedFile.type}</p>
            </div>
          </div>
          {!disabled && (
            <button onClick={onClear} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full text-slate-400 hover:text-red-500 transition-colors">
              <X size={20} />
            </button>
          )}
        </div>
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isDragging 
          ...
          : 'border-slate-300 dark:border-slate-600 hover:border-blue-400 hover:bg-slate-50 dark:hover:bg-slate-700/50 bg-white dark:bg-slate-800'
        }
    ...
    >
      <input 
        ...
        disabled={disabled}
      />
      <div className="p-4 bg-slate-100 dark:bg-slate-700 rounded-full mb-3 text-slate-500 dark:text-slate-400">
        <Upload size={24} />
      </div>
      <p className="text-slate-900 dark:text-slate-200 font-medium">點擊或拖放檔案以上載</p>
